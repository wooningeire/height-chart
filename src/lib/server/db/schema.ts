...
export const userTable = pgTable("user", {
	discordId: bigint({mode: "bigint"}).notNull().primaryKey(),
	discordAvatarUrl: varchar({length: 256}).notNull(),
	discordDisplayName: varchar({length: 256}).notNull(),
})
	.enableRLS();
...
export const characterTable = pgTable("character", {
	...
	ownerUserId: bigint({mode: "bigint"}).notNull().references(() => userTable.discordId),
})
	.enableRLS();
